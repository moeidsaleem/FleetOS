...
export default function UnderperformersPage() {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      ...
      try {
        ...
        setUnderPerformers(data.data.underPerformers || []);
      } catch (err) {
  ...
  return (
    <RequireAuth>
      <DashboardLayout>
        <PerformanceNav active="underperformers" />
        <div className="container mx-auto py-8">
          <h1 className="text-3xl font-bold mb-6">Underperformers</h1>
              ...
              <table className="min-w-full divide-y divide-border">
                <thead>
                  <tr>
                    ...
                    <th className="px-4 py-2 text-left text-xs font-bold">Alerts Pending</th>
                    <th className="px-4 py-2 text-left text-xs font-bold">Actions</th>
                  </tr>
                </thead>
                <tbody>
                  {underPerformers.length === 0 ? (
                    <tr><td colSpan={4} className="text-muted-foreground py-8 text-center">No underperformers found.</td></tr>
                  ) : (
                    underPerformers.map(driver => (
                      <tr key={driver.driverId} className="hover:bg-muted/40 transition-all">
                        ...
                        <td className="px-4 py-2">{driver.alertsPending || 0}</td>
                        <td className="px-4 py-2">
                          <button className="px-2 py-1 rounded bg-red-100 text-red-700 hover:bg-red-200">Send Alert</button>
                        </td>
                      </tr>
                    ))
                  )}
